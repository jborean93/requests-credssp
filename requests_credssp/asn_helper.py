...
def pack_asn1(data):
    """
    This method will pack the ASN.1 length and value elements for the value
    passed in.

    :param data: The data to pack in ASN.1 format
    :return: An ASN.1 encoded structure of the length and value of the data
        passed in
    """
    ...
    if value_length >= 0 and value_length <= 0x7f:
        ...
        length = struct.pack('!BBH', 0x83, value_length >> 16,
                             value_length & 0xffff)
    else:
...
def unpack_asn1(data):
    """
    This method will unpack the ASN.1 structure that is returned from the
    server.
    ...
    # Check if the first bit is set which means the format is long.
    # (CredSSP data is big endian, i.e. first bit is 7)
    is_long_format = is_set(first_length_octet_value, 7)

    if is_long_format:
        # length definition is set in multiple octets
        # Set the first bit of the first length octet and get it's value.
        # Add an extra value for the first octet
        length_octets = 1 + clear_bit(first_length_octet_value, 7)

        # Initialise empty variables for storing the length value.
        # current_offset is used to multiple each octet value
        value_octets = 0
        ...
        for i in range(length_octets - 1):
            ...
            # Decrement the current_offset so the next octet
            # gets the correct to power result
            current_offset -= 1
...
def get_context_field(field_info):
    """
    This method will create an ASN.1 field with the relevant context
    (sequence byte) set by the field_info value. This is then used in the full
    ASN.1 structure and then send to the server for validation.
    ...
    if field_value is None:
        if field_info.optional is False:
            raise AsnStructureException("Cannot get data for mandatory field "
                                        "%s, value is not set"
...
def parse_context_field(data, field_info):
    """
    This method will take in the decoded ASN.1 data and parse the next field in
    the sequence. It will update the field_info map with the value that is
    parsed.

    :param data: The raw ASN.1 data field to parse
    :param field_info: A ASN1Field object that holds the field metadata and
        data
    :return: The total bytes of the value, for the calling method to know the
        next offset point
    """
    ...
    if sequence_byte != expected_sequence_byte:
        raise AsnStructureException(
            "Expecting sequence (%x) for %s, was (%x)"
            % (expected_sequence_byte, field_name, sequence_byte))
    ...
    if type_byte != expected_type_byte:
        raise AsnStructureException(
            ...
            % (field_name, expected_type_byte, type_byte)
        )
    ...
    # Return the field value and the total bytes of the field + 1 to include
    # the sequence byte
    return total_bytes
...
class ASN1Field(object):
    def __init__(self, field_name, sequence, type, optional=False):
        """
        Generic ASN.1 field object to store common values for fields in a
        structure. Will set the value field to None on initialisation

        :param field_name: The human friendly name of the field, used in
            exception messages
        :param sequence: The sequence number of the field when used in a
            sequence (Also known as context)
        :param type: The ASN.1 field type of the field
        :param optional: Whether the field is optional (True) or
            mandatory (False)
        """
...
class ASN1Sequence(object):
    """
    Generic methods used by the ASN.1 structures in asn_structures. Generic
    boilerplate methods that can be used to parse data, get the data from the
    defined fields initialised in the structure.
    ...
    def __getitem__(self, item):
        if item in self.fields:
            ...
            raise AsnStructureException("Illegal field %s in ASN.1 structure"
                                        % item)
    ...
    def parse_data(self, data):
        """
        Parses an ASN.1 data structure returned from the server based on the
        fields initialised in the child class. Will assert that the sizes are
        correct and that the values set are a valid DER encoded ASN.1 structure

        :param data: The raw ASN.1 data returned from the server
        """
        type_byte = struct.unpack('B', data[:1])[0]
        if type_byte != self.type:
            error_msg = "Expecting %s type to be (%x), was (%x)"\
                        % (self.name, self.type, type_byte)
            raise AsnStructureException(error_msg)
    ...
    def get_data(self):
        """
        Creates an ASN.1 data structure based on the values already set in the
        object. Will throw an error in get_context_field is the value is not
        set and the field is mandatory.
