...
def parse_nt_status_exceptions(hex_code):
    """
    Loops through the exists NTStatusCode constants defined and will print out
    the exception details if found.

    :param code: The NTSTATUS hex code returned from the server under the
    TSRequest field errorCode
    """
    for status_name, status_value in vars(NTStatusCodes).items():
        if not status_name.startswith("__"):
            if hex_code == status_value:
                raise NTStatusException('%s - %s'
